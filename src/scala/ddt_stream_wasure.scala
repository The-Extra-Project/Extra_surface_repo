...
if (output_dir.isEmpty ||
  ...
  System.err.println("Bash variable are empy, please set it")
  System.exit(1)
}

// ===== Algo params extraction / struct def  ==========
// Xml stuff
if(!Files.exists(Paths.get(env_xml))){
  System.err.println("ERROR")
...
val dump_extra_steps_lvl = get_param_from_xml(env_xml, "dump_extra_steps_lvl", "2").toInt;
...
if(do_stream){
  params_wasure = params_wasure + ("do_stream" -> List("") )
  params_ddt = params_ddt + ("do_stream" -> List("") )
}

val do_plot = (dump_extra_steps_lvl > 1)
...
val ply2dataset_cmd =  set_params(params_ddt, List(("step","ply2data"))).to_command_line
...
val gc_on_stream_cmd =  set_params(params_wasure, List(("step","gc_on_stream"),("skip_app_header",""))).to_command_line
...
// val res_cpy = iq.run_pipe_fun_KValue(
//   copy_cmd ++ List("--label", "copy"),
//   kvrdd_points, "dim", do_dump = false).collect
// /home/laurent/shared_spark/output_wasure_2D/seg_ll_0_id4_20_03_2019_13_57_55.ply
// Delauay triangulatin computation
val (graph_tri,log_tri,stats_tri)  = ddt_algo.compute_ddt(kvrdd_points,iq,params_ddt,do_plot);
val graph_pts = Graph(kvrdd_dim, graph_tri.edges, List(""));

val stats_kvrdd = kvrdd_simplex_id(stats_tri.last,sc)
...
val input_seg_dst =  iq.aggregate_value_clique(graph_dst, 1);
...
if(false){
  val fill_graph_cmd =  set_params(params_wasure, List(("step","fill_graph"))).to_command_line
  val extract_graph_cmd =  set_params(params_wasure, List(("step","extract_graph"),("graph_type","2"))).to_command_line
  val input_seg = input_seg_dst.union(iq.aggregate_value_clique(graph_stats, 1)).reduceByKey(_ ::: _)
  val full_graph = iq.run_pipe_fun_KValue(
    extract_graph_cmd,
    input_seg, "ext_gr", do_dump = false)
  val  input_gc : RDD[KValue] = full_graph.map( x=> (1L,List(x))).coalesce(1)

  val gc_res = iq.run_pipe_fun_KValue(
    gc_on_stream_cmd,
    input_gc, "ext_gr", do_dump = false)
...
//val ll = 0.005;
val it_list = List(50)
//val lambda_list = List(0,0.005,0.01,0.1)
...
if(false){
  ...
  lambda_list.foreach{ ll =>
    ...
    params_wasure = params_wasure + ("lambda" -> List(fmt.format(ll)) )
    val input_seg = input_seg_dst.union(iq.aggregate_value_clique(graph_stats, 1)).reduceByKey(_ ::: _)
    val kvrdd_seg = compute_belief_prop(input_seg,it,0.001, stats_tri.last, params_wasure, iq, sc);
    val graph_seg = Graph(kvrdd_seg, graph_dst.edges, List(""));
    if (dim == 2)  {
      iq.run_pipe_fun_KValue(
        ply2geojson_cmd ++ List("--label","sparkcuted2_ll_" + fmt.format(ll),"--style","tri_seg.qml"),
        kvrdd_seg, "seg", do_dump = false).collect()
      // iq.run_pipe_fun_KValue(
      //   tri2geojson_wasure_cmd ++ List("--label","sparkcuted_it_" + fmt.format(it),"--style","tri_seg.qml"),
      //   iq.aggregate_value_clique(graph_seg, 1), "seg", do_dump = false).collect()g
    }
    val rdd_ply_surface = iq.run_pipe_fun_KValue(
      ext_cmd ++ List("--label","ext_spark_ll_" + fmt.format(ll) + "_it_" + fmt.format(it)),
...
lambda_list.foreach{ ll =>
  ...
  val res_seg = iq.run_pipe_fun_KValue(
    ...
    input_seg_dst, "seg", do_dump = false)
  ...
  if (dim == 2)  {
    iq.run_pipe_fun_KValue(
      ply2geojson_cmd ++ List("--label","segtri_ll_" + fmt.format(ll),"--style","tri_seg.qml"),
      kvrdd_seg, "seg", do_dump = false).collect()
    iq.run_pipe_fun_KValue(
