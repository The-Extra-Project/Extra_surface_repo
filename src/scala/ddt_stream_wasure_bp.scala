...
val extract_graph_cmd =  set_params(params_wasure, List(("step","extract_graph"),("lambda","10"))).to_command_line
...
val vertexRDD: RDD[(Long, PVertex)] = nodes_cpp.map { line =>
  ...
  val prior = Variable(fields.tail.map(x => -x.toDouble))
...
//beliefs.vertices.map( x => (x._1,x._2.belief)).collect.map(println(_))
...
val res_belief_str = res_belief.map(
  {case (u,v) => ((summed_id.indexWhere(x => x._2(2) > (u) )) - 1,(1,u.toString + " " + v.toString))  }
).reduceByKey(
  (v1,v2) => ((v1._1 + v2._1), v1._2 + " " + v2._2)
).map( x => (x._1.toLong, List("l 1 " + x._1.toString + " s " + x._2._1 + " " + x._2._2)) )

val input_extract_graph = (input_extract_simplex).union(res_belief_str).reduceByKey(_ ::: _);
...
val graph_seg = Graph(kvrdd_seg, graph_tri.edges, List(""));
  if (dim == 2)  {
    iq.run_pipe_fun_KValue(
      ply2geojson_cmd ++ List("--label","sparkcuted_ll_" + "1","--style","tri_seg.qml"),
      kvrdd_seg, "seg", do_dump = false).collect()
    iq.run_pipe_fun_KValue(
      tri2geojson_wasure_cmd ++ List("--label","sparkcuted_ll_" + "1","--style","tri_seg.qml"),
