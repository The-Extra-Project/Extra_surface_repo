...
val svrdd_dim = sc.objectFile[(String,IQlibCore.Value)](output_dir + "/*/kvrdd_dim")
val svrdd_simp = sc.objectFile[(String,IQlibCore.Value)](output_dir + "/*/kvrdd_simp")

val kmap  = svrdd_dim.map(x => x._1).zipWithUniqueId().collect().toMap

val kvrdd_dim = svrdd_dim.map(x => (kmap(x._1),x._2))
val kvrdd_simp = svrdd_simp.map(x => (kmap(x._1),x._2))

val  input_ddt = kvrdd_simp;
