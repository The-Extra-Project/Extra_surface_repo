...
function build {
    ...
    DDT_TRAITS="D2"
    while getopts "d:t:c:f:b:m:j:" OPTION; do
    ...
    # echo "------------------------------------------"
    # echo ""
    if [[ -z ${CURRENT_PROJECT_SRC_DIR} || -z ${CURRENT_PROJECT_BUILD_DIR} ]]; then
        echo "---- Err in $0 : bad args -----"
        echo "$0 build -d project_build (where CMakelist.txt)"
        exit 1
    fi

    echo -e "\n[$0] Installing/Updating ${CURRENT_PROJECT_SRC_DIR} ...\n"
    ...
    cmake -DCMAKE_BUILD_TYPE=$COMPILE_TYPE ${CMAKE_FLAGS} ${CURRENT_PROJECT_SRC_DIR} -DDDT_TRAITS=${DDT_TRAITS} && \
    make -j$NB_PROC
