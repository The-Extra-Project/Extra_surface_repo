...
for plyfile in ${OUTPUT_DIR}/colorized_ply/*.ply ; do
    while (( ${num_jobs@P} >= ${NUM_PROCESS} )); do
        wait -n
    done
    
    python  ${CUR_DIR}/colorize.py --input_mesh_dir ${OUTPUT_DIR}/outputs/tiles/ --input_pts ${plyfile} --output_colorized_mesh ${OUTPUT_DIR}/colorized_tiles &
done
