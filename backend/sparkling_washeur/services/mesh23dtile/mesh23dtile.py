...
def is_inside_bbox(bbox, pts):
    return (
        (pts[0] > bbox[0] and pts[0] < bbox[1])
        and (pts[1] > bbox[2] and pts[1] < bbox[3])
        and (pts[2] > bbox[4] and pts[2] < bbox[5])
    )
...
def trans_bbox(bb, coords):
    ...
    xmin_new, ymin_new, zmin_new = transformer.transform(
        bb[0] + x_shift, bb[2] + y_shift, bb[4]
    )
    xmax_new, ymax_new, zmax_new = transformer.transform(
        bb[1] + x_shift, bb[3] + y_shift, bb[5]
    )
...
def merge_subtree(node_tt, depth, output_dir, coords):
    ...
    if inputs["meshlab_mode"] == "python":
        ...
        ms.per_face_color_function(
            r=str(cc[0] * 255), g=str(cc[1] * 255), b=str(cc[2] * 255)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", default="", help="give the input ply dir")
    parser.add_argument("--output_dir", default="", help="give the output  dir")
    parser.add_argument("--bbox", default="", help="give bbox")
    parser.add_argument(
        "--meshlab_mode",
        default="python",
    ...
    parser.add_argument("--coords", default="", help="translation coords")
    parser.add_argument("--mode_proj", default="0", help="mode_proj")
