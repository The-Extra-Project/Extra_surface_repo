...
//           x => ((x.toString endsWith ".ply")) && ((ss_reg).findFirstIn(x.toString).isDefined)).size
...
//         val ss_reg = regexp_filter.r
//         val nb_file = fs.listStatus(new Path(input_dir)).map(x => fs.listStatus(x.getPath)).reduce(_ ++ _).map(_.getPath).filter(
//           x => ((x.toString endsWith ".stream")) && ((ss_reg).findFirstIn(x.toString).isDefined)).size
...
//           e => (e._2.toLong, List("z 1 " + e._2.toString + " z " + e._1.toString))).repartition(nb_file).setName("KVRDD_INPUT")
//       }
//       case _ => {
