...
# Update
- 2024-08-13 : Full pipeline dockerized and tested on example
...
### Build the docker image & compile the project
```console
...
# Run the code
## Examples 
To run the example, do in the current dir :
```console
CMD="/app/wasure/run_workflow.sh --input_dir ${PWD}/datas/lidar_hd_crop_1/ --output_dir ${PWD}/outputs_examples/lidar_hd_crop_1"
CMD_DOCKER="docker run -u 0 -v ${PWD}:${PWD} --rm -it ddt_img_base_devel /bin/bash -c \"${CMD}\""
eval $CMD_DOCKER

```
Results will be created in the 'outputs_examples' directory.
...
- To run the code on a LiDAR HD tile collection : 
  ...
  - then change the input dir of the previous example with the laz directory
...
## Working/debbuging
Instead of building the code inside the docker image,
```console
